...
void ROIsMonitor::drawRois(cv::Mat& image_cam, std::list<ROI>& listROIs, int focusedROI)
{
    ...
    while (it_roi != it_end)
    {
        ...
        int roiColor = tron::Draw::eYELLOW;        
        if (it_roi->getID() == focusedROI)
            roiColor = tron::Draw::eGREEN;                  
         
        oDraw.setDefaultColor(roiColor);
        oDraw.drawEllipse(it_roi->getPos(), roi_w, roi_h, -roi_angle);
...
//        int color = tron::Draw::eYELLOW;
//        if (absSpeed > 0.01)
//            color = tron::Draw::eRED;            
        oDraw.setDefaultColor(tron::Draw::eYELLOW);
        oDraw.drawPoint(it_roi->getPos(), 3);
...
void ROIsMonitor::drawFPS(float fps)
{
    ...
    oDraw.setDefaultColor(tron::Draw::eGREEN);    
