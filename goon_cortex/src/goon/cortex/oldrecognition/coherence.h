#ifndef __GOON_CORTEX_COHERENCE_H
#define __GOON_CORTEX_COHERENCE_H
...
public:
        Coherence(MemoryCenter* oMemoryCenter, int im_w, int img_h);
        ~Coherence();

    void checkSelfCoherence (int lmk_size, int* lmk_global_position, CvSeq* seq_candidate_matches);
    ...
    void checkGlobalCoherence (CvSeq* seq_lmk_context, CvSeq* seq_candidate_matches);
    // This function checks the global coherence of the candidate matches and updates their Coherence value.
    // It compares the positions of each landmark's neighbours with those of its candidate objects.
    // The final Coherence is affected by the sum of the self and context evidences.
