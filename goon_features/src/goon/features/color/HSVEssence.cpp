...
void HSVEssence::update(cv::Vec3f& hsvColor)
{
    int last_type = type;    
    hsv_main = hsvColor;
...
//    cv::split(grid_hsv, HSV_channels);
...
//    mat_dev = abs(mat_dev);
//    cv::Scalar sat_dev = cv::mean(mat_dev, grid_mask);
//
//    // value
//    mat_dev = grid_val * 255.0;
//    mat_dev = mat_dev - hsv_main[2];
//    mat_dev = abs(mat_dev);
//    cv::Scalar val_dev = cv::mean(mat_dev, grid_mask);
//
//    // hue
...
//    cv::Scalar hue_dev = cv::mean(mat_dev, grid_mask);   
//    
//    hsv_dev[0] = hue_dev(0);
//    hsv_dev[1] = sat_dev(0);
//    hsv_dev[2] = val_dev(0);
...
float HSVEssence::compare(cv::Vec3f& hsv_color)
{
    ...
    switch (type)
    {
        case eTYP_COLOR:
            ...
            dist1 = std::fabs(hsv_color[0] - hsv_main[0]);
            if (dist1 > 180)
                dist1 = 360 - dist1;
            dist1 = dist1 / HUE_DISC;
            dist2 = std::fabs(hsv_color[1] - hsv_main[1]) / SAT_DISC;
            dist3 = std::fabs(hsv_color[2] - hsv_main[2]) / VAL_DISC;
        ...
        case eTYP_GREY:
            ...
            dist2 = std::fabs(hsv_color[1] - hsv_main[1]) / SAT_DISC_GREY;
            dist3 = std::fabs(hsv_color[2] - hsv_main[2]) / VAL_DISC;
            dist = (dist2 > dist3 ? dist2 : dist3);
            break;
        
        case eTYP_DARK:
            dist = std::fabs(hsv_color[2] - hsv_main[2]) / VAL_DISC_DARK;
            break;
...
float HSVEssence::compare(cv::Vec3i& hsv_color)
{
    ...
    cv::Vec3f hsvColor2 = {(float)hsv_color[0], (float)hsv_color[1], (float)hsv_color[2]};
    return compare(hsvColor2);
}
